...
norm_image = cv2.normalize(sobel, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
...
while (1):        
    ...
    ret,qrgthreshold = cv2.threshold(norm_image ,magnitudevalue,255,cv2.THRESH_BINARY)
    cv2.imshow("Magnitude",qrgthreshold)
